...
def get_colormap_cache():
    return get_colormap(index)

@st.cache_data
def get_legend_cache():
    return get_legend_colormap(index, index_name)
...
with row1_col1:
    map = display_map(st.session_state.map_secrets, st.session_state.layer, available_layers[st.session_state.layer], get_colormap_cache())
