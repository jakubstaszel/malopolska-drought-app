...
st.write(
    "Turbidity is a parameter that illustrates water transparency. The value is primarily influenced by chlorophyll, suspended sediments and dissolved organic matter (TOPP et al., 2020). In addition to visual aspects that are important for recreational use of rivers and reservoirs, the transparency of water affects changes in the availability of light with the depth, which translates into photosynthesis process (Izagirre et al., 2009). In remote sensing studies, it can be used to assess lake trophism."
)
st.subheader("Turbidity Statistics Across Time")
...
st.subheader("Turbidity Spatial Variability")
widget = st.empty()
